...
typedef struct {
	...
	volatile tPoint point;					//Координаты последнего считанного касания (в координатах тачскрина)
	volatile uint32_t last_click_time; 		//Длительность последнего касания, выраженная в количестве прерываний системного таймера,
											//произошедших от момента последнего касания тачскрина до момента отпускания тачскрина
	volatile uint32_t click_time_counter;	//Счетчик длительности текущего касания (также выражается в количестве прерываний системного таймера)
	uint8_t fl_interrupt;					//Флаг разрешения обновления координат тачскрина в прерывании
	volatile uint8_t click;					//Флаг, показывающий, есть или нет в текущий момент касание тачскрина
	tCoef coef;								//Коэффициенты преобразования координат тачскина в дисплейные координаты
	volatile uint8_t fl_timer;				//Флаг, который разрешает или запрещает счет внутреннего таймера обработчика
	volatile uint32_t timer_counter;		//Счетчик внутреннего таймера обработчика
	uint32_t timer_period;					//Период счетчика внутреннего таймера обработчика
	volatile uint8_t fl_wait_exti;			//Флаг ожидания разрешения прерывания exti
	volatile XPT2046_status status;			//Статус опроса тачскрина: нет клика, клик, удержание
} XPT2046_Handler;
